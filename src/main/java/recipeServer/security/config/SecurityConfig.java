package recipeServer.security.config;

import com.nimbusds.jose.jwk.source.ImmutableSecret;
import recipeServer.security.error.CustomOAuth2AccessDeniedHandler;
import recipeServer.security.error.CustomOAuth2AuthenticationEntryPoint;
...
public class SecurityConfig {
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http, HandlerMappingIntrospector introspector) throws Exception {
    ...
    http.authorizeHttpRequests((authorize) -> authorize
            ...
            //Allow get methods for anonymous users
            ...
            //allow for users
            // I would like to add get,post,put and delete for users in 1 line, but idk how
            .requestMatchers(mvcMatcherBuilder.pattern(HttpMethod.GET, "/api/recipes")).hasAuthority("USERS")
            .requestMatchers(mvcMatcherBuilder.pattern(HttpMethod.PUT, "/api/recipes")).hasAuthority("USERS")
            .requestMatchers(mvcMatcherBuilder.pattern(HttpMethod.POST, "/api/recipes")).hasAuthority("USERS")
            .requestMatchers(mvcMatcherBuilder.pattern(HttpMethod.PATCH, "/api/recipes")).hasAuthority("USERS")
